...
TelloDriverNode::TelloDriverNode(const rclcpp::NodeOptions &options)
  ...
  command_srv_ = create_service<tello_msgs::srv::TelloAction>(
      "tello_action",
      std::bind(&TelloDriverNode::command_callback, this, std::placeholders::_1,
  ...
  spin_timer_ =
      create_wall_timer(1s, std::bind(&TelloDriverNode::timer_callback, this));

  // Parameters - Allocate the parameter context as a local variable because it
  // is not used outside this routine
...
#define CXT_MACRO_MEMBER(n, t, d)                                              \
  ...
  // NOTE: This is not setup to dynamically update parameters after ths node is
  // running.
...
void TelloDriverNode::timer_callback() {
  ...
  if (command_socket_->waiting() && now() - command_socket_->send_time() >
