...
void CommandSocket::timeout() {
  ...
  if (waiting_) {
    complete_command(tello_msgs::msg::TelloResponse::TIMEOUT,
...
void CommandSocket::process_packet(size_t r) {
  ...
  if (waiting_) {
    ...
    complete_command(str == "error" ? tello_msgs::msg::TelloResponse::ERROR
                                    : tello_msgs::msg::TelloResponse::OK,
                     str);
  } else {
