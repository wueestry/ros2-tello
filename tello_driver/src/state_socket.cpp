...
// * some future SDK version might introduce new field types, so don't parse
// undocumented fields
// * send the raw string as well

StateSocket::StateSocket(TelloDriverNode *driver, unsigned short data_port)
    : TelloSocket(driver, data_port) {
...
void StateSocket::process_packet(size_t r) {
  ...
  if (receiving_ && driver_->count_subscribers(
                        driver_->flight_data_pub_->get_topic_name()) == 0) {
    // Nothing to do
    return;
  }
  ...
  std::regex re("([^:]+):([^;]+);");
  for (auto i = std::sregex_iterator(raw.begin(), raw.end(), re);
  ...
  if (!receiving_) {
    ...
    RCLCPP_INFO(driver_->get_logger(), "Receiving state, SDK version %s",
                enum_names[sdk_].c_str());
  }

  // Only send ROS messages if there are subscribers
  if (driver_->count_subscribers(driver_->flight_data_pub_->get_topic_name()) >
      0) {
