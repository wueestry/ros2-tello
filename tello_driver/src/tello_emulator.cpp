...
const std::string FD_1_3{
    "pitch:1;roll:5;yaw:0;vgx:0;vgy:0;vgz:0;templ:43;temph:46;"
    "tof:10;h:0;bat:83;baro:150.12;time:0;agx:15.00;agy:-97.00;agz:-988.00;"};
const std::string FD_2_0{
...
void emulator(bool emulate_2_0, std::string name, unsigned short drone_port,
              unsigned short data_port, unsigned short video_port) {
  ...
  for (;;) {
    ...
    size_t length =
        command_socket.receive_from(asio::buffer(buffer), sender_endpoint);
    ...
    std::cout << name << " heard '" << command << "' from " << address << ":"
              << port << std::endl;
    ...
    if (command.rfind("sdk?", 0) == 0) {
      if (emulate_2_0) {
        command_socket.send_to(asio::buffer(std::string("20")),
                               sender_endpoint);
      } else {
        command_socket.send_to(
            asio::buffer(std::string("unknown command: sdk?")),
            sender_endpoint);
      }
    ...
    if (!connected && command == "command") {
      ...
      state_thread =
          std::thread([&state_socket, &state_remote_endpoint, flight_data]() {
            for (;;) {
              state_socket.send_to(asio::buffer(flight_data),
                                   state_remote_endpoint);
              sleep(1);
            }
          });
    }
    ...
    if (!streaming && command == "streamon") {
      ...
      video_thread = std::thread([&video_socket, &video_remote_endpoint]() {
        for (;;) {
          video_socket.send_to(asio::buffer(std::string("some video")),
                               video_remote_endpoint);
          sleep(1);
        }
      });
    }
...
int main(int argc, char *argv[]) {
  try {
    ...
    std::cout << name << " on 127.0.0.1:" << drone_port << ", data port "
              << data_port << ", video port " << video_port << std::endl;
