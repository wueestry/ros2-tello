...
class PyH264Decoder {
   ...
   */
  py::tuple decode_frame_impl(const ubyte *data, ssize_t num,
...
py::tuple PyH264Decoder::decode_frame_impl(const ubyte *data_in, ssize_t len,
  ...
  if (is_frame_available = decoder.is_frame_available()) {
    ...
    gilguard.lock();
    //   Construction of py::handle causes ... TODO: WHAT? No increase of ref
    //   count ?!
    py::object py_out_str(
        py::handle<>(PyString_FromStringAndSize(NULL, out_size)));
  ...
  try {
    while (len > 0) {
      ...
      try {
        ...
        if (num_consumed <= 0)
          // This case is fatal because we cannot continue to move ahead in the
          // stream.
          throw e;
      }
