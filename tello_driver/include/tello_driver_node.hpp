...
//
// FastRTPS also uses asio, and there's already an asio::io_service in the
// rclcpp::Node process. This can cause a deadlock. We avoid this by pushing the
// asio calls to the TelloSocket instances, which run in their own threads. See:
...
public:
  CommandSocket(TelloDriverNode *driver, std::string drone_ip,
...
private:
  ...
  std::vector<unsigned char>
      seq_buffer_;             // Collect video packets into a larger sequence
  size_t seq_buffer_next_ = 0; // Next available spot in the sequence buffer
  int seq_buffer_num_packets_ =
      0; // How many packets we've collected, for debugging
